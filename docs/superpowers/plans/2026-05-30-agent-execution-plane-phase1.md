...
cargo test -p aep-itest --test security_chain -- --ignored
```
Expected: 3 passed — `permitted_tool_runs`, `denied_tool_does_not_run`, `forged_capability_is_rejected_at_tool_boundary`. (Assertions are response-based so the three concurrent tests don't race on the shared counter; "runs exactly once" is covered by `effectively_once`.)
